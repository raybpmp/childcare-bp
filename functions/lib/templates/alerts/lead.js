...
const leadAlertTemplate = (data) => {
    ...
    const allData = Object.assign({ 'Email Address': data.email, 'Active Segment': data.funnelSegment, 'State / Location': data.state, 'Annual Revenue Potential': data.revenuePotential ? `$${data.revenuePotential.toLocaleString()}` : undefined, 'UTM Source': data.utmSource }, data.quizData);
